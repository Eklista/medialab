...
export interface DashboardDataTableProps<T> {
  ...
  stickyColumns?: boolean; // Nueva prop para activar columnas sticky
}
...
function DashboardDataTable<T>({
  ...
  sortDirection = 'asc',
  onSort,
  stickyColumns = true, // Por defecto activado
  ...
  // Effect para detectar cambios en el tamaño y scroll
  useEffect(() => {
  ...
  // Función para obtener clases de columnas sticky
  const getStickyColumnClasses = (position: 'first' | 'last', isHeader = false) => {
    if (!stickyColumns || !hasHorizontalScroll) return '';
    
    const baseClasses = 'sticky z-20';
    const shadowClass = position === 'first' 
      ? 'shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]' 
      : 'shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.1)]';
    
    const bgClass = isHeader ? 'bg-gray-50' : 'bg-white';
    const positionClass = position === 'first' ? 'left-0' : 'right-0';
    
    return `${baseClasses} ${positionClass} ${bgClass} ${shadowClass}`;
  };
      ...
      <div className="overflow-x-auto" ref={tableContainerRef}>
        <table className={tableClasses}>
          <thead className="bg-gray-50">
            <tr>
              {selectable && (
                <th className={`w-4 px-6 py-4 ${getStickyColumnClasses('first', true)}`}>
                  <input
                    type="checkbox"
              ...
              {columns.map((column, index) => {
                const isFirstColumn = !selectable && index === 0;
                const isLastColumn = index === columns.length - 1 && !actionColumn;
                
                let stickyClass = '';
                if (isFirstColumn) {
                  stickyClass = getStickyColumnClasses('first', true);
                } else if (isLastColumn) {
                  stickyClass = getStickyColumnClasses('last', true);
                }
              ...
              {actionColumn && (
                <th className={`px-6 py-4 text-right text-xs font-medium text-gray-500 uppercase tracking-wider ${getStickyColumnClasses('last', true)}`}>
                  Acciones
                </th>
              )}
            </tr>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map((item, index) => {
              ...
              const rowBgClass = isSelected ? 'bg-blue-50' : (striped && index % 2 === 1 ? 'bg-gray-50' : 'bg-white');
              ...
              return (
                ...
                >
                  {selectable && (
                    <td className={`px-6 py-4 ${getStickyColumnClasses('first')} ${rowBgClass}`}>
                      <input
                        type="checkbox"
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                        checked={isSelected}
                  ...
                  {columns.map((column, colIndex) => {
                    const isFirstColumn = !selectable && colIndex === 0;
                    const isLastColumn = colIndex === columns.length - 1 && !actionColumn;
                    
                    let stickyClass = '';
                    if (isFirstColumn) {
                      stickyClass = `${getStickyColumnClasses('first')} ${rowBgClass}`;
                    } else if (isLastColumn) {
                      stickyClass = `${getStickyColumnClasses('last')} ${rowBgClass}`;
                    }
                  ...
                  {actionColumn && (
                    <td className={`px-6 ${compact ? 'py-2' : 'py-4'} whitespace-nowrap text-right text-sm font-medium ${getStickyColumnClasses('last')} ${rowBgClass}`}>
